  ...
  return (
    <BasicModal open={open} setOpen={setOpen} contentMaxWidth={480}>
      ...
      <Typography variant="description" color="text.secondary" sx={{ mb: 5, display: 'block' }}>
        ...
        {rows.length > 1 && (
          <>
            {' '}
            <Trans>
              Each borrower is settled in its own transaction, so you will be asked to sign{' '}
              {rows.length} times.
            </Trans>
          </>
        )}
      ...
      >
        {rows.length > 1 ? (
          <Trans>
            {rows.length} transactions · {totalCalls} backstop matches
          </Trans>
        ) : (
          <Trans>Single transaction · {totalCalls} backstop matches</Trans>
        )}
      </Typography>
    </BasicModal>
  );
